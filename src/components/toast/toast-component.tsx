...
import React, { Fragment, useCallback, useEffect, useState, type ReactNode } from 'react'
...
export function ToastContainer() {
  ...
  const createNewToast = (newToast: Omit<IToast, 'id'>) => {
    const alreadyMessage = toasts.some((toast) => toast.message === newToast.message)
...
function Toast({
  ...
  return (
    <GestureDetector gesture={pan}>
      <Animated.View style={exitingStyles} layout={CurvedTransition.duration(350)}>
        <PressableAnimated
          entering={FadeInUp.delay(50).duration(350)}
          onLongPress={handleToastLongPress}
          exiting={SlideOutRight.duration(200)}
          onPress={onPress || undefined}
          style={[animatedSwipStyles, styles.toastContainer, { borderColor: feedbackColors[type] }]}
        >
          {autoHide && (
            <Animated.View
              style={[sliderStyle, styles.toastTime, { backgroundColor: feedbackColors[type] }]}
            />
          )}
...
function FeedbackIcon({ feedbackType }: { feedbackType: ToastFeedbackType }) {
  return (
    <View style={[styles.feedbackIcon, { backgroundColor: feedbackColors[feedbackType] }]}>
